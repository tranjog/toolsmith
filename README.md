...
| `web_fetch`  | GET a URL. Returns JSON verbatim; strips tags/scripts from HTML. 10k-char cap. For JSON, three optional extractors shrink the payload before truncation: `path` (single dot/bracket, e.g. `current.temperature_2m`), `paths` (array of paths → `{path: value}` map), or `filter` (jq program, e.g. `.daily \| {time, tmax: .temperature_2m_max}`). Use `filter` to reshape parallel arrays. |
